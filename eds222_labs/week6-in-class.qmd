...
x <- runif(10, 0, 10) # uniform distribution 
...
inv_logit <- function(x) exp(x)/ (1 + exp(x)) # inverse logit = logistic 
p <- inv_logit(logit_p)

# Simulate y
# Note: rbinom() with size = 1 is equivalent to a random Bernoulli variable
y <- rbinom(10, size =1, p) # r binomial, created our own bernolli function 
...
likelihood_fun <- function(beta0, beta1) {
  logit_p <- beta0 + beta1*x
  p <- inv_logit(logit_p)
  prod(dbinom(y, size = 1, p)) # telling us the value of pmf 
}

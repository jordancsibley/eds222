...
```         
pie_crab %>% 
  summary(lm(size~latitude), .)
```

The liner regression fit better the water temp data better in relation to latitude. The R squared is higher, which means there is a lesser spread of errors in comparison to the size variable.
...
ggplot(data = mpg, aes(y = hwy, x = year_fct)) +
  geom_boxplot() +
  labs(x = "Vintage year",
       y = "Highway miles per gallon")


# visualize the skew of density 
...
Intercept is the mean fuel efficiency when x is zero, or in this case when year is 1999 

The coefficient estimate on the 2008 is beta1. With a categorical variable (year 1999 or 2008), beta1 is the not really the slope, it is the difference relative to 1999, or the change. Based on lm, it is a 2% change in fuel efficiency from 1999 to 2008. 

...
1.  What do you think each row represents?\
    ...
    Each row represents the demographic info and number of drinking water violations per utility district of 4 different fiscal years (2010-2013)
...
```{r}
# Create new column that detemines if a violation occured or not 
drinking_water <- drinking_water %>% 
  mutate(violation = ifelse(health > 0, 1, 0)) # vectorized function 
```
...
4.  Create a scatter plot of violations against race (percent Black population), ethnicity (percent Hispanic population), and SES (median household income). What visualization issues do we get with a scatter plot? How could you address that?\
    ...
    The points are overplotted. We can summarize within bins to view the patterns

```{r}
# Make df to view data clearer
...
drinking_water_complete %>% 
  ...
  ggplot(aes(y = residual, x = pcthisp)) +
  geom_point()
```
...
```{r}
logit_p_lower <- (beta0_hat + beta1_hat* c(10, 45, 80))
p_low <- inv_logit(logit_p_lower)

logit_p_higher <- (beta0_hat + beta1_hat* c(20, 55, 90))
p_high <- inv_logit(logit_p_higher)
...
5.  Create a "DEM" of the likelihood landscape for $\beta_0$ and $\beta_1$. Choose a range of $\beta_0$ and $\beta_1$ values around your best estimates, calculate the likelihood for each combination, and create a figure with $\beta_0$ on the x-axis, $\beta_1$ on the y-axis, and the likelihood as the fill. Add a point for $(\hat\beta_0, \hat\beta_1)$.\
    ...
    \
    Notes on apply functions

    sapply: Works on vectors and creates a new vector that has been altered by the function

    mapply: multiple vectors then gives you one output. multiple application
...
This tells us that beta 0 is more sensitive to change than beta 1. Our model is very sensitive to the intercept we choose. Our estimate for beta 1, our likelihood wouldn't change that much. Confidence interval for beta 1 is much larger than it is for beta 0 
...
predictions <- expand_grid(
  ...
  mutate(violations = predict(interaction_glm, 
                              newdata = ., # tells to look above to predictions 
                              type = "response"))

...
```{r}
# load in pie crab data 
lter <- lterdatasampler::pie_crab

# look at variable names 
#View(lter)

# see if lat is associated with size of pie crab ('latitude', 'size')
size<- ggplot(data = lter) +
  geom_point(aes(x = latitude, y = size)) +
  labs(x = "Latitude",
       y = "Body size of PIE crab")

water<- ggplot(data = lter) +
...
Remember: do not put the variables in quotes `" "`, ggplot will read this as a string and not the variable
...
(ii) water temperature and latitude: -0.7
...
cor(x = lter$latitude, y = lter$water_temp)
# -0.957
```

SHORTHANDS for saying these symbols out loud
...
```{r}
# y ~ x
summary(lm(size~latitude, data = lter))
# intercept B0 = -3.624 
# slope b1 = 0.485

lm1<-lm(size~latitude, data = lter)

summary(lm(water_temp~latitude, data = lter))
# Intercept (B0) = 48.77
# Slope B1 = -0.77
...
ggplot(data = lter, aes(x = latitude, y = size)) +
  ...
  labs(x = "Latitude",
       y = "Body size of PIE crab") 
# default for formula in geom_smooth is y ~ x
